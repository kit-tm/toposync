...
/**
 * Superclass for the TopoSync-SFC and reference formulations. Implements common constraints and adds the tree
 * constraints per template methods which can then be implemented in the concrete classes.
 */
        ...
        for (NprTraffic noSfcFlow : trafficNoSfc) {
            ...
            for (TopologyVertex dst : noSfcFlow.getEgressNodes()) {
                print("delay for " + dst.toString() + ":" +
                              topoSyncPlacementSolver.getDelayForFlowAndDestination(noSfcFlow, dst)
                                                     .get(GRB.DoubleAttr.X));
            }
