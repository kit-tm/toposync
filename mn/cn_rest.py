...
class CnRESTRequestHandler( BaseHTTPServer.BaseHTTPRequestHandler ):
    ...
    def _instantiate(self, dpid, vnf_type):
        ...
        if vnf_type.endswith("_accelerated"):
            cmd_string = "python %smn/hosts/cli/RAW/Sniffer.py %s %s %s &" % (PATH_TO_MN, (host_name + '-eth0'), 100, vnf_type)
            print("starting sniffer: %s: %s" % (host_name, cmd_string))
        else:
            cmd_string = "python %smn/hosts/cli/RAW/Sniffer.py %s %s %s &" % (PATH_TO_MN, (host_name + '-eth0'), 400, vnf_type)
            print("starting sniffer: %s: %s" % (host_name, cmd_string))
